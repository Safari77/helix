...
impl Document {
    ...
    > {
        ...
        let future = async move {
            ...
            // 1. DIRECTORY CREATION AND CHECKS
            ...
            // Check if file exists to determine hardlink status
            ...
            let try_atomic = atomic_save && !is_hardlink;

            // 3. ATTEMPT TO CREATE FILE (ATOMIC OR DIRECT)
            let mut temp_path: Option<PathBuf> = None;
            let mut file_handle: fs::File;

            if try_atomic {
                // Prepare data needed for the blocking closure
                ...
                // Direct Strategy (Hardlink or Atomic Config off)
                file_handle = fs::File::create(&write_path).await?;
            }
            ...
            // 5. FINALIZE ATOMIC SAVE (COPY METADATA & RENAME)
